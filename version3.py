...
def display_menu(menu):
    print("\n" + "=" * 60)
    print("           Welcome to the School Canteen")
    print("=" * 60)

    print(f"{'No.':<5}{'Item':<25}{'Price':<10}{'Stock'}")
    print("-" * 60)

    # Sort alphabetically
    menu.sort(key=lambda item: item["name"])

    for i, item in enumerate(menu, start=1):
        print(f"{i:<5}{item['name'].title():<25}${item['price']:<9.2f}{item['stock']}")
